...
class GraphicsEngine {
  ...
  async _drawTextGradient(text, position, baseColor, alignment /* effects */) {
    ...
    // Top: bright, Middle: normal, Bottom: slightly darker
    const colors = [
      [Math.min(255, r + 30), Math.min(255, g + 30), Math.min(255, b + 30), a], // Bright top
      [r, g, b, a], // Normal middle
      [Math.max(0, r - 40), Math.max(0, g - 40), Math.max(0, b - 40), a], // Darker bottom
    ];
