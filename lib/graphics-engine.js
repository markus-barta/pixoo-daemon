...
class GraphicsEngine {
  ...
  async _drawTextGradient(text, position, baseColor, alignment /* effects */) {
    ...
    // Simple gradient: darken bottom of text
    const gradientSteps = 3;
    for (let step = 0; step < gradientSteps; step++) {
      const factor = 1 - step * 0.2; // Gradually darken
      const gradientColor = [
        Math.round(r * factor),
        Math.round(g * factor),
        Math.round(b * factor),
      ...
      await this.device.drawText(text, [x, y + step], gradientColor, alignment);
    }
  }
  ...
  async drawGradientBackground(startColor, endColor, direction = 'vertical') {
    ...
    if (direction === 'vertical') {
      ...
      for (let y = 0; y < 64; y++) {
        ...
        const b = Math.round(startB + (endB - endB) * factor);
