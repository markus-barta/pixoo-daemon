...
class SceneManager {
  ...
  /**
   * Resume a paused scene (restarts the loop)
   * @param {string} host - Device host/IP
  ...
  async resumeScene(host, context) {
    try {
      ...
      if (playState !== 'paused') {
        this.logger.warn(
          `Scene is not paused for ${host} (state: ${playState})`,
        );
        return false;
      }
      ...
      const gen = this._getDeviceState(host, 'generationId');
      const deviceLoop = async () => {
        // Abort if scene changed or paused again
        const active = this._getDeviceState(host, 'activeScene');
  ...
  /**
   * Stop the current scene completely (cleanup and remove)
   * @param {string} host - Device host/IP
   * @param {Object} context - Scene context
   * @returns {Promise<boolean>} Success status
   */
  async stopScene(host, context) {
    try {
      const activeScene = this._getDeviceState(host, 'activeScene');
      if (!activeScene) {
      ...
      // Cleanup the scene
      try {
        await this.cleanupScene(activeScene, context);
      } catch (error) {
        this.logger.warn(`Cleanup failed for ${activeScene}:`, error.message);
      }

      // Clear device state
      this._setDeviceState(host, 'activeScene', null);
      this._setDeviceState(host, 'status', 'stopped');
      this.devicePlayState.set(host, 'stopped');

      this.logger.ok(`⏹️  Stopped scene: ${activeScene} (host=${host})`);
      return true;
    } catch (error) {
