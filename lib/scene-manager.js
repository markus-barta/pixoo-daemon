...
class SceneManager {
  ...
  constructor({ logger, stateStore } = {}) {
    ...
    this.deviceStatus = new Map(); // host -> 'switching'|'running'|'stopping' (fallback)
  ...
  getSceneInternalState(host) {
    ...
    if (!currentScene || !this.stateStore) {
      this.logger.warn(
        `[DEBUG] getSceneInternalState(${host}): no scene or stateStore`,
      );
      return {};
    }
    const snapshot = this.stateStore.getSceneSnapshot(host, currentScene) || {};
    this.logger.debug(
