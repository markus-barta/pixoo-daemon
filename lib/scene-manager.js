...
class SceneManager {
  ...
  constructor({ logger, stateStore } = {}) {
    ...
    // NOTE: We keep these Maps even with StateStore for now to avoid refactoring
    // all the existing code. Full StateStore integration is TODO for Phase 2.
    ...
    if (this.stateStore) {
      this.logger.debug(
        'SceneManager initialized with StateStore (future use)',
      );
    }
  }
