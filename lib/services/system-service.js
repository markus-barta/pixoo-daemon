...
class SystemService {
  ...
  /**
   * Restart the daemon using process spawning (more reliable than wrapper)
  ...
  async restartDaemon() {
    try {
      ...
      // For Docker environments, use the wrapper script approach
      if (
        process.env.NODE_ENV === 'production' ||
        process.env.DOCKER_CONTAINER
      ) {
        this.logger.info('Using Docker wrapper restart method');
        ...
        // For development/local environments, try to restart the process directly
        this.logger.info('Attempting direct process restart');

        setTimeout(() => {
          try {
            // Spawn a new instance of the daemon
            ...
            child.on('error', (err) => {
              this.logger.error('Failed to spawn new daemon process:', {
                error: err.message,
              });
            });
            ...
            child.on('spawn', () => {
              this.logger.ok(
                'New daemon process spawned, exiting current process...',
              );
            ...
            this.logger.error(
              'Direct restart failed, falling back to exit method:',
              {
                error: err.message,
              },
            );
            // Fallback to exit method
            setTimeout(() => {
              process.exit(42);
            }, 500);
          }
