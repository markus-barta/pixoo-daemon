...
class SystemService {
  ...
  async restartDaemon() {
    try {
      ...
      // Graceful shutdown signal with non-zero exit code for Docker restart
      setTimeout(() => {
        this.logger.ok('Initiating daemon restart...');
        process.exit(1); // Non-zero exit code signals Docker to restart
      }, 1000);
