...
class SceneService {
  ...
  /**
   * Stop the current scene on a device
   * @param {string} deviceIp - Device IP address
   * @returns {Promise<Object>} Result with success status
   */
  async stopScene(deviceIp) {
    try {
      ...
      const success = await this.sceneManager.stopScene(deviceIp, context);
      ...
      this.logger.ok(`Successfully stopped scene on ${deviceIp}`);
