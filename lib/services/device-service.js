...
class DeviceService {
  ...
  async setDisplayPower(deviceIp, on) {
    try {
      ...
      // Use the device's brightness method (0 = off, 100 = full brightness)
      if (typeof device.setBrightness === 'function') {
        await device.setBrightness(on ? 100 : 0);
        ...
        throw new ValidationError('Device does not support brightness control');
      }
    } catch (error) {
