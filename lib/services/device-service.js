...
class DeviceService {
  ...
  async setDisplayPower(deviceIp, on) {
    try {
      // Use Channel/SetIndex to turn display on/off
      // SelectIndex: 3 = Custom (on), 4 = Black screen (off)
      ...
      // Re-render current scene if turning on
      if (on) {
        const sceneState = this.sceneManager.getDeviceSceneState(deviceIp);
        if (sceneState.currentScene && sceneState.currentScene !== 'none') {
          this.logger.info(
            `Re-rendering ${sceneState.currentScene} after turning display on`,
          );
  ...
  /**
   * Reset a device (soft reset)
   * @param {string} deviceIp - Device IP address
   * @returns {Promise<Object>} Result with success status
   */
  async resetDevice(deviceIp) {
    try {
      this.logger.warn(`Resetting device: ${deviceIp}`);

      const success = await this.softReset(deviceIp);

      if (!success) {
        throw new Error('Soft reset returned false');
      }
