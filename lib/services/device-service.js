...
class DeviceService {
  ...
  async setDisplayPower(deviceIp, on) {
    try {
      ...
      if (driver === 'mock') {
        this.logger.info(
          `[MOCK] Would turn display ${on ? 'ON' : 'OFF'} for ${deviceIp}`,
        );
      } else {
  ...
  async setDisplayBrightness(deviceIp, brightness) {
    try {
      ...
      if (driver === 'mock') {
        this.logger.info(
          `[MOCK] Would set brightness to ${brightnessValue}% for ${deviceIp}`,
        );
      } else {
        const { httpPost } = require('../pixoo-http');
        await httpPost(deviceIp, {
  ...
  async resetDevice(deviceIp) {
    try {
      ...
      if (driver === 'mock') {
        this.logger.info(
          `[MOCK] Would reset device ${deviceIp} (showing init screen)`,
        );
      } else {
