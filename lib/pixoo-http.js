...
class RealPixoo {
  ...
  async drawImageWithAlpha(path, pos, size, alpha = 255) {
    ...
    // 1. Load the image file from disk
    // 2. Decode the image format (PNG, etc.)
    // 3. Apply alpha blending
    // 4. Draw each pixel to the buffer
    ...
    logger.debug(
      `Drew image placeholder at [${x},${y}] size [${width}x${height}] alpha ${alpha}`,
    );
  }
}
